...
class DownloadEssential():
    def VerifyIfFileNotExists(self):
        if self.convert:
            ...
            return not(exists(f"{self.path}/Músicas/{filename}.mp3"))
        else:
            filename = self.stream.default_filename
            return not(exists(f"{self.path}/Músicas/{filename}"))

    def ConvertToMp3(self):
        filename = self.stream.default_filename.replace('.mp4', '')
        os.rename(f"{self.path}/Músicas/{filename}.mp4", f"{self.path}/Músicas/{filename}.mp3")
